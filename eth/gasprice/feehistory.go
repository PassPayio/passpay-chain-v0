...
func (oracle *Oracle) FeeHistory(ctx context.Context, blocks uint64, unresolvedLastBlock rpc.BlockNumber, rewardPercentiles []float64) (*big.Int, [][]*big.Int, []*big.Int, []float64, error) {
	if blocks < 1 {
		return common.Big0.ToBig(), nil, nil, nil, nil // returning with no data and no error means there are no retrievable blocks
	}
	...
	for i, p := range rewardPercentiles {
		if p < 0 || p > 100 {
			return common.Big0.ToBig(), nil, nil, nil, fmt.Errorf("%w: %f", errInvalidPercentile, p)
		}
		if i > 0 && p <= rewardPercentiles[i-1] {
			return common.Big0.ToBig(), nil, nil, nil, fmt.Errorf("%w: #%d:%f >= #%d:%f", errInvalidPercentile, i-1, rewardPercentiles[i-1], i, p)
		}
	}
	var (
		pendingBlock    *types.Block
	...
	if err != nil || blocks == 0 {
		return common.Big0.ToBig(), nil, nil, nil, err
	}
	...
	for ; blocks > 0; blocks-- {
		fees := <-results
		if fees.err != nil {
			return common.Big0.ToBig(), nil, nil, nil, fees.err
		}
	...
	if firstMissing == 0 {
		return common.Big0.ToBig(), nil, nil, nil, nil
	}
	if len(rewardPercentiles) != 0 {

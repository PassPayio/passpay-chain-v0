...
// Package ppos implements the proof-of-stake-authority consensus engine.
package ppos
...
// Ppos proof-of-stake-authority protocol constants.
var (
...
var (
	getblacklistTimer = metrics.NewRegisteredTimer("ppos/blacklist/get", nil)
	getRulesTimer     = metrics.NewRegisteredTimer("ppos/eventcheckrules/get", nil)
)
...
// Ppos is the proof-of-stake-authority consensus engine proposed to support the
// Ethereum testnet following the Ropsten attacks.
type Ppos struct {
	chainConfig *params.ChainConfig // ChainConfig to execute evm
	config      *params.PposConfig  // Consensus engine configuration parameters
...
// New creates a Ppos proof-of-stake-authority consensus engine with the initial
// validators set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Ppos {
	// Set any missing consensus parameters to their defaults
	conf := *chainConfig.Ppos
	if conf.Epoch == 0 {
...
func (c *Ppos) SetChain(chain consensus.ChainHeaderReader) {
	c.chain = chain
}

// SetStateFn sets the function to get state.
func (c *Ppos) SetStateFn(fn StateFn) {
	c.stateFn = fn
}

// Author implements consensus.Engine, returning the Ethereum address recovered
// from the signature in the header's extra-data section.
func (c *Ppos) Author(header *types.Header) (common.Address, error) {
...
func (c *Ppos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Ppos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Ppos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Ppos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Ppos) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Ppos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Ppos) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (c *Ppos) TrySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	// Now it should be PPT vote token increase
	state.AddBalancePPT(header.Coinbase, fee)
	// reset fee
...
func (c *Ppos) doSomethingAtEpoch(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) ([]common.Address, error) {
	newSortedValidators, err := c.getTopValidators(chain, header)
	if err != nil {
		return []common.Address{}, err
	}
...
func (c *Ppos) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
...
func (c *Ppos) updateValidators(vals []common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
...
func (c *Ppos) punishValidator(val common.Address, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
	method := "punish"
...
func (c *Ppos) decreaseMissedBlocksCounter(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	// method
...
// with.
func (c *Ppos) Authorize(validator common.Address, signFn ValidatorFn, signTxFn SignTxFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Ppos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: val}, accounts.MimetypePpos, PposRLP(header))
	if err != nil {
		return err
	}
...
func (c *Ppos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Ppos) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for ppos as there are no background threads.
func (c *Ppos) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the validator voting.
func (c *Ppos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "ppos",
		Version:   "1.0",
		Service:   &API{chain: chain, ppos: c},
		Public:    false,
	}}
}
...
// PposRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func PposRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (c *Ppos) PreHandle(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
...
func (c *Ppos) IsSysTransaction(sender common.Address, tx *types.Transaction, header *types.Header) (bool, error) {
	if tx.To() == nil {
		return false, nil
	}
...
func (c *Ppos) CanCreate(state consensus.StateReader, addr common.Address, height *big.Int) bool {
...
func (c *Ppos) ValidateTx(sender common.Address, tx *types.Transaction, header *types.Header, parentState *state.StateDB) error {
...
func (c *Ppos) getBlacklist(header *types.Header, parentState *state.StateDB) (map[common.Address]blacklistDirection, error) {
...
func (c *Ppos) CreateEvmExtraValidator(header *types.Header, parentState *state.StateDB) types.EvmExtraValidator {
	if c.chainConfig.SophonBlock != nil && c.chainConfig.SophonBlock.Cmp(header.Number) < 0 {
...
func (c *Ppos) getEventCheckRules(header *types.Header, parentState *state.StateDB) (map[common.Hash]*EventCheckRule, error) {
	defer func(start time.Time) {
...
func (c *Ppos) getEventCheckRulesLen(header *types.Header, parentState *state.StateDB) (int, error) {
...
func (c *Ppos) commonCallContract(header *types.Header, statedb *state.StateDB, contractABI abi.ABI, addr common.Address, method string, expectResultLen int, args ...interface{}) ([]interface{}, error) {

...
var (
	beaconDifficulty = common.Big0.ToBig()  // The default block difficulty in the beacon consensus
...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
	...
	if diff := new(big.Int).Sub(header.Number, parent.Number); diff.Cmp(common.Big1.ToBig()) != 0 {
...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction, uncles []*types.Header, withdrawals []*types.Withdrawal, _ *[]*types.Receipt, _ []*types.Transaction) error {
	if !beacon.IsPoSHeader(header) {
		beacon.ethone.Finalize(chain, header, state, txs, uncles, nil, nil, nil)
		return errors.New("pos header error")
	}
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal) (*types.Block, []*types.Receipt, error) {
	...
	beacon.Finalize(chain, header, state, &txs, uncles, withdrawals, nil, nil)
